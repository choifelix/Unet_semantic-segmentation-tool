...
def plot_images(in_img, out_img, label_img):
    """
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % 100 == 0:
            ...
            if visual :
                ...
                output_img = segmented_img(output, treshold)
                label_img  = format_label_img(target)
                
                plot_images(input_img, output_img, label_img)
                
        torch.cuda.empty_cache()


def test(model, device, test_loader, loss_fn, visual=False):
    model.eval()
    test_loss = 0
