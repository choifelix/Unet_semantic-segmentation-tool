...
class Encoder(nn.Module):
    def __init__(self, input_size, stage_sizes=[64,128,256,512,1024]):
        super().__init__()
        self.conv_blocks = nn.ModuleList([ConvBlock(input_size, stage_sizes[0])] \
                                         + [ConvBlock(stage_sizes[i], stage_sizes[i+1]) for i in range(len(stage_sizes)-2)]
                                         + [ConvBlock(stage_sizes[-2], stage_sizes[-1], max_pooling = False)])
...
class Unet_model(nn.Module):
    ...
    """
    def __init__(self, input_size, stage_sizes, head_filter, nb_classes):
        """
        Arguments:
        ...
        self.encoder = Encoder(input_size , stage_sizes)
